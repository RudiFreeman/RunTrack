...
/** Группирует пробежки по дате, возвращает суммарную дистанцию (км) по дням */
...
/** Темп по забегам для линейного графика (только последние N) */
function buildPaceByRun(runs: SavedRun[]): { x: number; y: number }[] {
  ...
  const last = sorted.slice(-15); // максимум 15 точек
  return last.map((r, i) => ({ x: i + 1, y: parseFloat((r.avgPace / 60).toFixed(2)) }));
}

/** Максимальное количество забегов за любую неделю */
function maxRunsPerWeek(runs: SavedRun[]): number {
  if (runs.length === 0) return 0;
  const map: Record<string, number> = {};
  for (const r of runs) {
    const d = new Date(r.date);
    // ISO week key: год-неделя
...
export function StatsScreen() {
  ...
  // Личные рекорды — по всем пробежкам
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        {!hasEnoughData ? (
          <View style={styles.emptyBlock}>
            <Text style={styles.emptyText}>
              Нужно минимум {MIN_RUNS_FOR_CHARTS} забега для графиков
            </Text>
            <Text style={styles.emptySubText}>
              {filtered.length === 0
                ? 'В этом периоде нет пробежек'
                : `Пока ${filtered.length} из ${MIN_RUNS_FOR_CHARTS}`}
            </Text>
          </View>
        ) : (
          <>
            {/* Дистанция по дням */}
            <SectionTitle title="Дистанция по дням (км)" />
            <View style={styles.chartCard}>
              ...
              >
                ...
                <VictoryBar
                  ...
                  style={{
                    data: {
                      fill: ({ datum }) => (datum.y > 0 ? '#00E5A0' : '#1E1E1E'),
                      opacity: 0.9,
                    },
                  }}
                  cornerRadius={{ top: 3 }}
            ...
            {/* Темп по забегам */}
            <SectionTitle title="Темп по забегам (мин/км)" />
            <View style={styles.chartCard}>
              <VictoryChart
                width={CHART_WIDTH}
                height={CHART_HEIGHT}
              ...
              >
                ...
                <VictoryLine
                  data={paceData}
                  style={{
                    data: { stroke: '#00E5A0', strokeWidth: 2 },
                  }}
                  animate={{ duration: 400, onLoad: { duration: 400 } }}
                />
                <VictoryScatter
        ...
        <SectionTitle title="Личные рекорды" />
        {!hasAnyRun ? (
          <View style={styles.emptyBlock}>
            <Text style={styles.emptyText}>Пока нет пробежек. Выходи бегать! 🏃</Text>
          </View>
        ) : (
          <View style={styles.recordsGrid}>
            ...
            <RecordCard
              icon="📏"
              label="Длинный забег"
              value={longestRun > 0 ? formatDistance(longestRun) : '—'}
            />
            <RecordCard icon="🔥" label="Забегов за неделю" value={String(maxWeekly)} />
          </View>
        )}
